...
TIT="QA20015 Snap"; PASTA=tw.ROOT/"evidencias"/"val_retrabalhos_grupoD"
...
with tw.sync_playwright() as p:
    ...
    try:
        ...
        # esperar 55s pro processamento e capturar
        ...
        cd.scroll_into_view_if_needed(); cd.click(timeout=8000,force=True); page.wait_for_timeout(6000)
        # dump iframes + texto do preview
        info=page.evaluate("""()=>{const ifr=[...document.querySelectorAll('iframe')].map(f=>(f.src||'').slice(0,80)).filter(Boolean);
            const corpo=document.body.innerText.replace(/\s+/g,' ');
            return {iframes:ifr, proc:/em processamento|logo estará/i.test(corpo), vazio:/Não há conteúdo/i.test(corpo)};}""")
        print("INFO:",info)
        tw.snap(page,PASTA,"vid-snap-final",full=True)
