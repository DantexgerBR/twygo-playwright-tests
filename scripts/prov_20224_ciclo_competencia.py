"""20224 (v2): ciclo+campanha de COMPETENCIAS (SEM modelo) com Julia (autoavaliacao)."""
import sys, re
...
with tw.sync_playwright() as p:
    ...
    page.locator('[data-test-id="cycle-form-evaluation-card-competency"]').first.click(); page.wait_for_timeout(1000)
    page.locator('[data-test-id="cycle-form-tab-stages"]').click(); page.wait_for_timeout(1000)
    page.get_by_text("Auto-avaliação", exact=True).first.click(force=True); page.wait_for_timeout(700)
    page.get_by_text("Cálculo automático ponderado", exact=True).first.click(force=True); page.wait_for_timeout(700)
    ...
    tw.snap(page, PASTA, "25-ciclo-comp-criado", full=True)
    ...
    if cid:
        ...
        print("tipos campanha:", [*dict.fromkeys(tids)])
        comp = page.locator('[data-test-id="campaign-form-evaluation-type-competency"]')
        if comp.count(): comp.click(); print("tipo competency marcado")
        ...
        tw.snap(page, PASTA, "25b-campanha-comp-criada", full=True)
