...
| TC9 | critical | FALHOU | Gestor de turma (qalider@teste.com, com organograma e liderado1 criados) vê a org inteira: KPIs 260/13/80/13 idênticos ao Admin, lista com usuários fora do time (qa11tc342816). liderado1 não aparece na lista sem busca e busca retorna "Não há dados" para "liderado". Escopo do líder não está restrito aos liderados diretos (RN 6.2). **Bug P1 ref: novo retrabalho — ver abaixo** |
...
**Resultado geral da execução (ATUALIZADO 2026-06-23 — TC9 re-executado)**: CONCLUÍDA — **5 PASSOU | 7 FALHOU | 0 BLOQUEADO**  
(TC2 reconciliado FALHOU→PASSOU-com-ressalva: divergência de label/coluna configurável, não bug. Os TCs com falha real viram retrabalhos; TC9 re-executado após criação de organograma — FALHOU: escopo do Gestor de turma não restrito ao time.)
...
## TC9 — Resultado da re-execução (2026-06-23)

Organograma criado por Dante: líder `qalider@teste.com` com liderado direto `liderado1@teste.com`.

**Fluxo testado**:
1. Admin confirmou 1 registro de liderado1 na org.
2. qalider logou, mudou para perfil "Gestor de turma" via seletor de perfil.
3. Acessou `/o/37079/records` como Gestor de turma.
4. KPIs na visão Gestor: 260 Emitidos, 13 Expirados, 80 Pendentes, 13 Recusados — idênticos à org inteira (Admin view).
5. Lista sem busca mostra `qa11tc342816@twygotest.com` (fora do time do líder) em todas as 25 linhas visíveis.
6. Busca por "liderado" → "Não há dados para exibir" (liderado1 não aparece).
7. Busca por `qa11tc342816` → "Não há dados para exibir" (resultado inconsistente com a lista sem busca — bug da busca P1/TC4 também afeta Gestor).
...
:: Validação ::
Suíte QA 1.2 concluída integralmente (TC9 re-executado em 2026-06-23): 5 PASSOU | 7 FALHOU | 0 BLOQUEADO.
...
- TC5: colunas Checkbox/Ações não sticky (position=static) e tabela sem altura fixa 660px/scroll interno (RN 9/RN 10 confirmados no Discovery).
- TC6: 3o clique no header não limpa o sort (asc→desc→asc, não asc→desc→none; RN 11 confirmado). Bug global Aluno+Admin.
- TC8: seleção de card no grid não dá borda roxa (border cinza) — RN 14.
- TC9: Gestor de turma vê a org inteira — KPIs 260/13/80/13 idênticos ao Admin; liderado1 não aparece na lista; usuários fora do time listados sem filtro. Escopo do líder não aplicado (RN 6.2). Retrabalho novo.
- TC11: hamburger ausente no mobile Admin — P2 #33118785 (bug global Aluno+Admin).
:: Obs ::
TC9 re-executado após criação de organograma (qalider → liderado1). Perfil "Gestor de turma" testado. Falha: API não filtra por liderados diretos, exibe org inteira.
